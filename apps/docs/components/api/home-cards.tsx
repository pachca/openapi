      ...
      {cards.map((card, idx) => (
        ...
        >
          <div className="flex justify-between items-start mb-3">
            <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-xl font-bold ${card.color}`}>
              {card.letter}
            </div>
            <ArrowUpRight className="w-4 h-4 text-text-tertiary group-hover:text-primary transition-colors" />
          </div>
          <h3 className="text-base font-medium text-text-primary mt-0! mb-1! group-hover:text-primary transition-colors">{card.title}</h3>
          <p className="text-[14px]! text-text-secondary! leading-snug mb-0! group-hover:text-primary transition-colors">{card.description}</p>
        </a>
      ))}
    </div>
  );
