...
<Limit
  title="Список чатов с постраничной пагинацией"
  limit="~10 запросов"
  period="1 секунда"
  howItWorks="Действует на <code>GET /chats</code>, когда выборка запрошена номером страницы, а не курсором. Считаем по токену авторизации. <span class='text-text-secondary'>Запрос с <code class='text-text-secondary!'>cursor</code> или <code class='text-text-secondary!'>limit</code> под этот лимит не подпадает и работает в рамках общего.</span>"
/>

<Limit
  title="Сообщения в один чат за месяц"
  limit="30 000 сообщений"
  period="Календарный месяц"
  entity="chat_id"
  howItWorks="Действует на <code>POST /messages</code>. Считаем по паре «отправитель и чат» за календарный месяц по UTC. Сообщения в тред засчитываются в чат, которому принадлежит родительское сообщение. <span class='text-text-secondary'>Квоту расходуют только созданные сообщения: запрос, отклонённый проверкой, её не тратит.</span>"
/>
