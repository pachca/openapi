...
description: "Журнал аудита событий Пачки для команд безопасности: структура записи, типы событий и состав деталей, фильтры и пагинация, хранение. Тариф «Корпорация»"
related:
  - { path: /guides/permissions, title: Права и роли }
  - /guides/dlp
  - /guides/export
  - { path: /guides/data-protection, title: Шифрование и безопасный контур }
---

# Журнал аудита событий

<Warning>Доступно только на тарифе **Корпорация**. Токену нужен скоуп `audit_events:read`, который доступен только владельцу пространства.</Warning>
...
Записи возвращает метод [Журнал аудита событий](GET /audit_events). Журнал доступен только на чтение, изменить или удалить запись нельзя.
...
- Так же логируется инвентаризация пространства: [Список чатов пространства](GET /company/chats) пишет `company_chats_accessed`, [Список ботов пространства](GET /company/bots) — `company_bots_accessed`. В обоих случаях актором и объектом события выступает владелец токена.
...
<Info>Идентификаторы верхнего уровня (`actor_id`, `entity_id`) приходят строками, а внутри `details` — числами. Это разные представления одного и того же идентификатора, приводите их к одному типу перед сравнением.</Info>
...
Полные схемы с типами и описанием каждого поля — в разделе «Ответ» на странице метода [Журнал аудита событий](GET /audit_events), в поле `details`.
