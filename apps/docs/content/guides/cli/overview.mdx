...
<Steps>
  ...
  <Step title="Вход в аккаунт">
    Если вход выполняете вы сами, достаточно подтвердить его в браузере — получать токен заранее не нужно. Для CI и агентов, где подтверждать некому, передайте готовый токен со страницы **Интеграции** > **API**.
