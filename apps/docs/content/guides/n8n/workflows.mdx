...
description: "Готовые сценарии автоматизации Пачки в n8n: приветствие сотрудника, пересылка сообщений, задачи из обсуждений, согласование, мониторинг и заявки на отпуск"
related:
...
1. **Pachca Trigger** — событие `Chat Member Added`
2. **Pachca** (Message > Create) — отправка приветствия в тот же чат: `entityId` берётся из `chat_id` события
...
<Info>Подробнее о кнопках — в разделе [Кнопки в сообщениях](/guides/n8n/advanced#knopki-v-soobscheniyah), о формах — в разделе [Формы](/guides/n8n/advanced#formy).</Info>
