...
export function generatePHP(endpoint: Endpoint, baseUrl: string = 'https://api.pachca.com/api/shared/v1'): string {
...
function phpRepr(obj: any, indent: number = 0): string {
  ...
  if (typeof obj === 'object') {
    ...
    const items = keys.map(key => `${nextIndentStr}'${key}' => ${phpRepr(obj[key], indent + 1)}`).join(',\n');
    return `[\n${items}\n${indentStr}]`;
  }
