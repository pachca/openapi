...
> Краткое содержание: Создаёт бота в пространстве и сразу отдаёт его accesstoken
...
Создаёт бота в пространстве и сразу отдаёт его `access_token`. Сохраните токен при создании: тот же самый повторно не показывают, останется либо выпустить новый на вкладке «API» в настройках бота, либо перевыпустить его методом [Ротация токена бота](/api/bots/recreate-token).
...
Входящий вебхук бота публикует сразу во все беседы и каналы, где бот состоит, поэтому набор его чатов и решает, куда попадут сообщения.
