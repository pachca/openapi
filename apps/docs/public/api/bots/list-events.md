...
> Краткое содержание: Метод работает только с токеном бота: у личного токена своего журнала нет
...
Сам журнал не очищается: прочитанные события удаляйте методом [Удаление события](/api/bots/remove-event), иначе они придут снова при повторном запросе.
