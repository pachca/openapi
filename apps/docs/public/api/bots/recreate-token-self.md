...
Ротируется основной токен бота — обычно это тот самый токен, которым сделан запрос, и он инвалидируется сразу. Дополнительные токены, выпущенные боту на странице «API», продолжают работать.
