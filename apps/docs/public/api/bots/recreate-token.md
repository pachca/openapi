...
Новый токен возвращается в ответе только один раз — сохраните его сразу. Права у него те же: скоупы переносятся с прежнего токена, заново их выдавать не нужно.

Меняется только основной токен бота. Дополнительные токены, выпущенные боту на странице «API», продолжают работать, и интеграции на них ротация не затрагивает.

Ротация доступна создателю бота, а также администратору пространства, если настройки бота открывают администраторам доступ к редактированию. Владельцу чата, где состоит бот, — нет, даже когда остальные настройки бота ему открыты.
