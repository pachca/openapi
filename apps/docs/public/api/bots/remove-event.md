...
> Краткое содержание: Метод работает только с токеном того бота, которому принадлежит событие.
