...
> Краткое содержание: Меняет настройки бота: перезаписываются только переданные поля.
