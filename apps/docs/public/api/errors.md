...
> Код `409` возвращается, когда объект с такими данными уже есть: сотрудник с этим адресом почты, тег с этим названием, закрепление у этого сообщения. В `errors[].code` приходит `already_exists`, а в `errors[].key` — конфликтующее поле: например, `email` при повторном адресе почты сотрудника. Если определить поле не удалось, `key` в ошибке не будет.


> Ответ `429 Too Many Requests` приходит в двух видах. Превышение месячного предела сообщений в чат — обычный `ApiError` с кодом `rate_limit`. Превышение лимита на частоту запросов обрабатывается до того, как запрос доходит до метода: тело не `JSON`, а короткий текст с `Content-Type: text/plain`. Разбирайте тело `429` только после проверки `Content-Type`, иначе разбор упадёт. Заголовок `Retry-After` есть в обоих случаях. Подробнее — на странице [Лимиты](/api/limits).
