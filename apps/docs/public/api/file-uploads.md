...
> Краткое содержание: Трёхшаговая загрузка файлов в API Пачки через presigned URL S3: получение подписи, отправка multipart/form-data и прикрепление к сообщению или задаче
...
Ссылка бывает и другой. Если в пространстве включено шифрование или доступ к вложениям ограничен доверенными сетями, `url` ведёт не в хранилище, а на адрес API. Такой файл скачивается с заголовком `Authorization`, токену нужен скоуп `files:read`, а запрос должен приходить с доверенного адреса. Отличить случай можно по самой ссылке, заранее знать настройки пространства не нужно.
