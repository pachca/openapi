...
Запрос уходит не в API Пачки, а прямо в хранилище, и в ответ приходит `204` без тела. Дальше файл прикладывают, передав его `key` в поле `files` — в [Новое сообщение](/api/messages/create) или [Новый черновик](/api/drafts/create), а также при их правке. Имя файла в `name` должно иметь то же расширение, что и `key`, иначе сообщение будет отклонено.
