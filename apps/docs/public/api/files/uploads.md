...
Ничего в Пачке при этом ещё не создаётся. Загруженный файл становится вложением только тогда, когда его `key` передан в поле `files` метода [Новое сообщение](/api/messages/create) — до этого он никому не виден и ни к чему не привязан.
