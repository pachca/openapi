...
Превышение месячного лимита отвечает так же — `429` с кодом `rate_limit` в `errors[].code` и заголовком `Retry-After`. Отправка в этот чат приостанавливается на час, и каждая следующая попытка во время паузы удваивает её: 1, 2, 4, 8, 16 и до 24 часов. Дождитесь срока из `Retry-After`, иначе пауза продолжит расти.
