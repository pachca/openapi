...
> Краткое содержание: Убирает сотрудника или бота из беседы, канала или треда
...
Сотрудника, которого держит в чате привязанный тег, этим методом убрать нельзя: ответ придёт успешный, а сотрудник останется в чате. Снимите с него тег методом [Редактирование сотрудника](/api/users/update) или отцепите тег от чата методом [Исключение тега](/api/members/remove-group-tag).

Если пользователь является владельцем чата, то исключить его нельзя. Он может только самостоятельно выйти из чата, воспользовавшись методом [Выход из чата](/api/members/leave).
