...
> Краткое содержание: Отправляет сообщение в беседу или канал, личное сообщение пользователю или комментарий в тред.
...
Тред, в отличие от личного чата, сам не появляется: если у сообщения треда ещё нет, его сначала создают методом [Новый тред](/api/threads/add).
...
Под текстом сообщения можно показать кнопки — массив `buttons`, где каждый вложенный массив это ряд. Работают они только у сообщений, отправленных токеном бота: нажатие на кнопку с `data` приходит событием `button_click` тому самому боту, а вместе с ним `trigger_id` — на открытие формы по нему есть 3 секунды, метод [Открытие представления](/api/views/open). Кнопка с `url` просто ведёт по ссылке и события не создаёт. Подробности в разделе [Кнопки](/guides/buttons).
...
- `message: object` (required) — Собранный объект параметров создаваемого сообщения
  ...
  - `entity_id: integer, int32` (required) — Идентификатор того, что названо в `entity_type`: беседы или канала, треда либо пользователя. Пример: `334`
  ...
  - `files: array of object` — Прикрепляемые файлы
    ...
    - `name: string` (required, max length: 255) — Название файла, которое вы хотите отображать пользователю. Расширение должно совпадать с расширением файла в `key`, иначе ответ будет 422 с кодом `wrong_params`. Пример: `"logo.png"`
  ...
  - `parent_message_id: integer, int32` — Идентификатор сообщения, на которое отвечаете. Ответ появится прямо в ленте чата, а не в треде: для треда отправляйте сообщение с `entity_type: "thread"`. Пример: `194270`
