...
> Краткое содержание: Сообщение вместе с автором, вложениями и кнопками.
...
Удалённое сообщение метод всё равно отдаёт — с пустым `content` и заполненным `deleted_at`, без файлов и реакций. Этим он отличается от метода [Список сообщений чата](/api/messages/list), где удалённых нет вовсе.
