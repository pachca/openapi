...
Меняет текст, файлы и кнопки уже отправленного сообщения. В интерфейсе у него появляется пометка о правке. Прежний текст ни через API, ни в приложении посмотреть нельзя.
