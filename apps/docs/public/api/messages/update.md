...
Меняет текст, файлы и кнопки уже отправленного сообщения. В интерфейсе у него появляется пометка о правке. Текст оформляется по тем же правилам, что и в методе [Новое сообщение](/api/messages/create).
