...
> Краткое содержание: Пагинация в API Пачки по курсору: две группы методов (списочные и поиск) с разной структурой meta, поля next_page, prev_page, has_next, has_prev, обход всех записей и polling новых данных через prev_page
...
- [Методы поиска](#metody-poiska) — возвращают `meta` с полем `total` и `paginate.next_page` (только курсор «вперёд», без `prev_page`/`has_next`/`has_prev`). К этой группе по поведению также относится [Список сотрудников](/api/users/list) с заданным параметром `query` — `meta` возвращается в упрощённом формате.

## Параметры запроса

| Параметр | Тип | Описание |
|----------|-----|----------|
| `limit` | integer | Количество записей на странице. Допустимые значения зависят от метода: большинство списочных методов — 1–50, [Список прочитавших сообщение](/api/read-member/list-readers) — до 300, [Поиск чатов](/api/search/list-chats) — до 100, [Поиск сотрудников](/api/search/list-users) и [Поиск сообщений](/api/search/list-messages) — до 200. |
...
К этой группе относятся: [Список сотрудников](/api/users/list) (без `query`), [Список чатов](/api/chats/list), [Список участников чата](/api/members/list), [Список сообщений чата](/api/messages/list), [Список реакций](/api/reactions/list), [Список прочитавших сообщение](/api/read-member/list-readers), [Список тегов сотрудников](/api/group-tags/list), [Список сотрудников тега](/api/group-tags/list-users), [Список напоминаний](/api/tasks/list), [Журнал аудита событий](/api/security/list), [История событий](/api/bots/list-events).
...
Поведение полей `meta.paginate` для типичных сценариев пагинации (на примере [Списка чатов](/api/chats/list) с DESC-сортировкой):
...
- [Список сотрудников](/api/users/list) **с заданным параметром `query`** — возвращает упрощённый `meta` без `prev_page`/`has_next`/`has_prev`. Если вызывать `/users` без `query`, ответ соответствует группе «Списочные методы» с полным `meta.paginate`.
