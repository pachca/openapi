...
> Краткое содержание: Карточка владельца токена
...
Чужую карточку отдаёт [Информация о сотруднике](/api/users/get).
