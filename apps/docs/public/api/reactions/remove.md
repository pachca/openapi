...
> Краткое содержание: Снимает с сообщения свою реакцию.
...
Удалять можно только те реакции, которые были поставлены авторизованным пользователем. Чужая реакция и реакция, которой нет, отвечают `404`. Подписку на тред, если она появилась вместе с реакцией, снятие реакции не отменяет.
...
- `code: string` (required) — Emoji символ реакции. Именно символ: текстовые записи вроде `+1` или `:+1:` отклоняются как неизвестный эмодзи.
