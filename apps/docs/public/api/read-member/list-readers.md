...
> Краткое содержание: Кто из участников чата дочитал ленту до этого сообщения
...
Имён метод не отдаёт. За ними ходят методом [Информация о сотруднике](/api/users/get) по каждому идентификатору.
