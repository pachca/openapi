...
Напоминание можно привязать к беседе полем `chat_id` — тогда его видят участники этой беседы, а привязывающий должен сам в ней состоять. В канале, в треде и в архивном чате напоминания не создаются.

Передать существующее напоминание другому сотруднику — это метод [Редактирование напоминания](/api/tasks/update), а не новое напоминание: копия с тем же текстом оставит исходное на прежнем ответственном.
