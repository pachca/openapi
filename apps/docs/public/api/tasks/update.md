...
Передать напоминание другому сотруднику — это тоже правка: добавьте его в `performer_ids` вместо прежнего. Написать ему сообщение вместо этого не поможет, напоминание останется на старом ответственном.

Доступно автору и ответственным. Участник чата, где висит напоминание, его видит, но менять не может. Привязку к чату этим методом не изменить — поле `chat_id` метод не принимает.
