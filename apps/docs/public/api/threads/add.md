...
> Краткое содержание: Заводит тред под сообщением — место, где обсуждают сказанное, не засоряя ленту чата
...
Пустой тред разговором ещё не является: сразу отправьте в него первое сообщение методом [Новое сообщение](/api/messages/create) с `entity_type: "thread"`. Названия у треда нет, и удалить его нельзя ни через API, ни в приложении.

Если у сообщения уже был создан тред, то в ответе на запрос вернётся информация об уже созданном ранее треде. Но чтобы просто прочитать тред, метод не нужен: идентификатор существующего треда приходит в поле `thread` самого сообщения. Тред под сообщением, которое само лежит в треде, завести нельзя: вложенных тредов в Пачке нет.
