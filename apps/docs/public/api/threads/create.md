...
Заводит тред, который ни к чему не привязан: место для разговора, диалога с агентом или звонка, которому не нужен отдельный чат. Собрать в нём можно любых сотрудников, и каждый увидит только сам тред.

Тело запроса не требуется. Создатель треда становится его единственным участником, а поля `message_id` и `message_chat_id` в ответе возвращаются как `null`. Остальных добавляют методом [Добавление пользователей](/api/members/add) по `chat_id` треда.
