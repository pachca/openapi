...
> Краткое содержание: Карточка сотрудника.
...
Мульти-гостю метод отдаёт только тех, кто приходит ему в [Список сотрудников](/api/users/list), а на остальных отвечает `404`.

Почта и телефон приходят `null`, если в пространстве выключен показ персональных данных, а вызывающий не администратор. Мульти-гостю они всегда приходят `null`. Свою карточку сотрудник видит целиком — она же отдаётся методом [Свой профиль](/api/profile/get) и без скоупа `users:read`.
