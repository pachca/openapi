...
> Краткое содержание: Ставит статус другому сотруднику — например, «в отпуске» на время отсутствия
...
Свой статус ставят методом [Новый свой статус](/api/profile/update-status) — он доступен всем ролям и ботам.
...
- `status: object` (required)
  ...
  - `expires_at: date-time` — Срок жизни статуса (ISO-8601). Смещение вида `+03:00` учитывается, время без смещения считается UTC. Сюда идёт срок, названный словами: «до трёх» это значение поля, а не часть подписи. Пример: `"2024-04-08T10:00:00.000Z"`
