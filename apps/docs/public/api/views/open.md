...
> Краткое содержание: Показывает сотруднику модальное окно с формой в ответ на его действие
...
Показывает сотруднику модальное окно с формой в ответ на его действие. Окно открывается от имени того, кому пришло нажатие кнопки, а события о нажатиях получают боты — поэтому на практике метод зовут с токеном бота.

Открыть окно можно единственным способом — по `trigger_id` из события о нажатии кнопки. Один `trigger_id` открывает одно окно и живёт 3 секунды с момента, когда Пачка передала боту нажатие, а у бота с журналом событий — с момента записи нажатия в журнал. Поэтому окно открывают первым делом, а всё остальное по событию делают уже после.

Заполненная форма приходит обратно событием `view_submit`: на исходящий вебхук, а если адрес не задан — в журнал, который отдаёт метод [История событий](/api/bots/list-events), и тогда ответом служит [Ответ на отправку формы](/api/views/submit-response).
