...
Самый быстрый и производительный способ. Главное: агенту **не нужно грузить документацию в контекст или ходить на сайт**. В CLI встроены команды со списком всех эндпоинтов и справкой по каждому (данные генерируются из OpenAPI-спецификации, из которой собран CLI) — агент запрашивает у CLI нужный метод и сразу его вызывает. Точечно, минимум контекста, всегда синхронно с актуальным API.
...
Команды списка и справки (`api ls` / `--describe` / `--spec` / `--docs`, нюанс с методом) — подробно в разделе [Запросы к API](/guides/cli/api-requests).
