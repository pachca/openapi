...
- [CLI](/guides/cli) — Все API-методы доступны как команды в терминале. Агент может вызывать их через shell.
...
AI-агенты с доступом к терминалу (Claude Code, Codex, собственные агенты) могут вызывать команды [CLI](/guides/cli) напрямую — без написания HTTP-запросов. Каждый API-метод доступен как команда с типизированными флагами.
