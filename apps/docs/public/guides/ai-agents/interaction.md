...
- [Добавление реакции](/api/reactions/add) — поставить реакцию-индикатор
...
- **Черновик вместо отправки** — когда цена ошибки высока, ответ можно не отправлять, а положить сотруднику в поле ввода, чтобы он дописал и отправил сам. Так агент действует не ботом, а личным токеном сотрудника — подробнее в разделе [Черновик вместо отправки](/guides/ai-agents/drafts).

## Реакция agent-thinking

Создайте [кастомную реакцию](https://pachca.com/help-center/features/kastomnye-reakcii) с именем `agent-thinking` и загрузите анимированную иконку. Когда агент ставит эту реакцию на сообщение, интерфейс Пачки автоматически показывает живой таймер вместо счётчика — пользователь видит, сколько секунд агент уже работает. Таймер отображается до 5 минут. В методе [Добавление реакции](/api/reactions/add) передайте параметр `name` со значением `agent-thinking`.
