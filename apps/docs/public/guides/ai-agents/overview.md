...
Полная версия документации в одном файле — [llms-full.txt](/llms-full.txt). Это объёмный файл: его можно загрузить в контекст агента целиком, но для постоянной работы это не лучший первый выбор — для неё предпочтителен CLI (см. ниже).
