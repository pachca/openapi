...
- Через API — метод [Добавление пользователей](/api/members/add), передав `user_id` бота в `member_ids`.
...
- Бот сам ранее отправил пользователю [Новое сообщение](/api/messages/create) через API (с `entity_type: "user"`) — после этого пользователь может отвечать, даже если бот не публичный.
...
- **Гости и мульти-гости** — видят **только тех, с кем уже есть личные сообщения**. Бот появится в выдаче только если с ним уже была переписка — например, потому что бот сам ранее отправил пользователю [Новое сообщение](/api/messages/create).
