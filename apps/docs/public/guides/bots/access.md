...
- Через API — метод [Добавление участника в чат](/api/members/add), передав `user_id` бота в `member_ids`.
...
- Через API — метод [Добавление участника в чат](/api/members/add), передав `chat_id` треда.
...
- Бот сам ранее [отправил сообщение](/api/messages/create) пользователю через API (с `entity_type: "user"`) — после этого пользователь может отвечать, даже если бот не публичный.
...
- **Гости и мульти-гости** — видят **только тех, с кем уже есть личные сообщения**. Бот появится в выдаче только если с ним уже была переписка — например, потому что бот сам ранее [отправил сообщение](/api/messages/create) пользователю.
