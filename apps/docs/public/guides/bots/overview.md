...
Метод [Новое сообщение](/api/messages/create) поддерживает поля `display_name` и `display_avatar_url`, позволяющие указать произвольное имя и аватар отправителя для конкретного сообщения. Бейдж **Бот** при этом сохраняется, а при открытии профиля отправителя пользователь увидит оригинальное имя и аватар бота.
