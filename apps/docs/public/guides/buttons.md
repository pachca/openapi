...
Для отправки используйте метод [Новое сообщение](/api/messages/create) с полем `buttons`. Поле представляет собой массив строк, каждая из которых содержит массив кнопок. В одной строке можно комбинировать URL-кнопки и Data-кнопки.
...
- [Отредактировать сообщение](/api/messages/update) — убрать кнопки, добавить новые или обновить текст
- [Отправить новое сообщение](/api/messages/create) — как ответ пользователю с результатом действия
...
- У кнопки заполнено ровно одно из полей `url` и `data`. Оба поля сразу или ни одного — ошибка `wrong_params`
