...
Не нужно знать эндпоинт заранее — CLI описывает API сам, прямо против своей спецификации:
...
Метод указывается позиционно, поэтому `--describe`/`--spec`/`--docs` всегда однозначны. Для агента это значит: не нужно грузить всю документацию в контекст или ходить на сайт — он спрашивает у самого CLI ровно нужный метод и сразу его вызывает (точечно, минимум контекста, всегда синхронно с актуальным API).
