...
Войти в CLI можно двумя способами: через браузер или передав готовый токен. Первый подходит, когда вход выполняет сотрудник, второй — CI и агентам, где подтверждать некому. Типы токенов, скоупы и настройка доступа описаны в руководстве [Авторизация](/api/authorization).
...
Каждый вход виден в списке **Токены CLI** на странице **Интеграции** > **API** под именем устройства. CLI передаёт имя машины, а для профиля не по умолчанию добавляет его имя в скобках — например, `macbook (ci)`, — поэтому два входа с одного ноутбука в списке различаются. Рядом показаны время входа и последнего вызова, а отзыв из списка завершает вход только на выбранном устройстве.
...
Персональный токен создаётся на странице **Интеграции** > **API**, токен бота — на вкладке **API** в карточке бота.
...
Такой токен не обновляется. Это единственный путь для CI и скриптов: без сотрудника у терминала подтвердить вход в браузере невозможно, поэтому `pachca auth login` без `--token` в неинтерактивном режиме сразу отвечает ошибкой.
...
- **Вход через браузер.** Токен гасится на сервере сразу — доступ закрывается в момент выхода, а не по истечении срока, и вход пропадает из списка **Токены CLI**. Если сервер в этот момент недоступен, CLI об этом скажет: профиль всё равно удаляется, а токен доживает свой час. Завершить вход можно и без терминала — кнопкой **Отозвать** в списке **Токены CLI**, например на потерянном ноутбуке.
- **Готовый токен.** Продолжит работать — он бессрочный. Удалите его на странице **Интеграции** > **API** или, если это токен бота, в карточке бота, когда он больше не нужен.
