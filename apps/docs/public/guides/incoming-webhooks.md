...
Шаблонизатор разбирает входящий JSON и подставляет значения полей в шаблон. Для форматирования текста используйте markdown:
...
| Ссылка | `[текст](https://url)` | [текст](https://url) |
| Строчный код | `` `код` `` | `код` |
| Блок кода | ` ``` код ``` ` | блок с подсветкой |
