> Краткое содержание: Unfurl в Пачке: превью ссылок внутренних сервисов прямо в чатах — бот ловит URL, подтягивает заголовок, описание, изображение и отправляет обратно в чат
...
Откройте **Интеграции** > **Боты** и нажмите **Создать бота**. Подробнее — в разделе [Создание бота](/guides/bots/setup).
  ...
  ### Шаг 2. Включите исходящий вебхук

В карточке бота на вкладке **Исходящий вебхук** нажмите **Включить** и укажите URL вашего сервера. Подробнее — в разделе [Исходящие вебхуки](/guides/webhook/overview).


  ### Шаг 3. Включите событие и укажите домены

В списке событий в группе **Ссылки** включите **Отправка ссылок в сообщении (unfurl)** и перечислите через запятую до 5 доменов, ссылки на которые бот будет отслеживать. Бот получает ссылки из всех чатов пространства, поэтому добавлять его в чаты не нужно, а включить событие и задать домены может только администратор или владелец пространства.


  ### Шаг 4. Обработайте вебхук

При появлении ссылки на отслеживаемый домен бот получит вебхук с массивом `links`. Каждый объект содержит `url`, `domain` и `skip: boolean` — если `skip` равен `true`, автор сообщения скрыл превью для этой ссылки и бот не должен создавать превью. Извлеките данные из вашего сервиса по оставшимся URL.


  ### Шаг 5. Отправьте превью

Вызовите метод [Unfurl (разворачивание ссылок)](/api/messages/unfurl), передав заголовок, описание и изображение для каждой ссылки.
...
Для создания превью используйте метод [Unfurl (разворачивание ссылок)](/api/messages/unfurl), передав `message_id` из вебхука.
...
- **Загрузкой файла** — параметр `image` с данными, полученными через метод [Загрузка файла](/api/files/direct-url). Если указаны оба параметра, `image` является приоритетным.

> **Внимание:** Если среди переданных URL будет ошибка (URL отсутствует в сообщении или домен не указан в настройках бота), запрос не будет выполнен — не будет создано ни одного превью, а в ответе придёт `wrong_params`.

  Ссылка, для которой автор сообщения скрыл превью, — не ошибка: она пропускается, а остальные превью из того же запроса создаются.
