...
Ресурс **Message** с операцией **Unfurl** позволяет формировать кастомные превью для ссылок в сообщениях бота.

Когда бот отправляет сообщение со ссылкой, Пачка может запросить у бота данные для превью. Бот может ответить через [Unfurl (разворачивание ссылок)](/api/messages/unfurl) с заголовком, описанием и изображением.
