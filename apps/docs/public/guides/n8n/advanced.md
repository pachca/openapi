> Расположение: n8n
> Краткое содержание: Продвинутые функции n8n-расширения Пачки: загрузка файлов через S3, интерактивные кнопки, формы, AI-агент, разворачивание ссылок и журнал безопасности
...
Ресурс **Chat** (операции **Request Export** и **Download Export**) позволяет выгружать сообщения из чатов. Экспорт выполняется асинхронно: вы запрашиваете экспорт, а Пачка присылает уведомление на вебхук, когда архив готов.
...
В основном workflow добавьте узел **Pachca** с ресурсом **Chat** и операцией **Request Export**. Укажите период (`startAt`, `endAt`) и вставьте URL вебхука из первого шага в поле `webhookUrl`.
...
В workflow с Webhook-узлом добавьте узел **Pachca** с ресурсом **Chat** и операцией **Download Export**. Передайте `export_id` из данных вебхука в поле **ID**. Архив будет скачан автоматически.
...
3. Бот вызывает [Открытие представления](/api/views/open) с `trigger_id` и описанием формы
...
Ресурс **Message** с операцией **Unfurl** ставит к ссылкам в сообщениях превью с заголовком, описанием и изображением — например, к ссылкам на задачи во внутреннем трекере.

Сценарий начинается с события **Link Shared** в узле **Pachca Trigger**: бот получает его, когда в любом чате пространства появляется ссылка на один из его доменов. Домены и само событие задаются на вкладке **Исходящий вебхук** в карточке бота, и включить их может только администратор или владелец пространства. Дальше workflow собирает данные по ссылке и отвечает операцией **Unfurl** — методом [Unfurl (разворачивание ссылок)](/api/messages/unfurl) по `message_id` из события.
