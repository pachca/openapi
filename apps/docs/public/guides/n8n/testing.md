> Расположение: n8n
> Краткое содержание: Тестирование n8n-узлов Пачки: Listen for test event, Pin Data и Execute Step для экшн-узлов, защита webhook-слота бота и локальная разработка через туннель
...
> В **n8n Cloud** Production URL и Test URL триггера публичные «из коробки» — туннель не нужен. Этот раздел касается только self-hosted-инсталляций на локальной машине.
