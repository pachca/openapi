...
4. Для токена бота: значение показывается только при создании, поэтому потерянный токен перевыпустите в карточке бота на вкладке **API** (**Перевыпустить** в меню токена)
5. Для персонального токена: создайте новый на странице **Интеграции** → **API**
...
|----------|-----------------|
| Управление сотрудниками (User > Create/Update/Delete) | `users:create`, `users:update`, `users:delete` (доступны администраторам и владельцам) |
| Журнал безопасности (Security > Get Many) | `audit_events:read` (доступен только владельцу пространства) |
...
| Отправка сообщений (Message > Create) | `messages:create` |
| Чаты (Chat > Create/Update) | `chats:create`, `chats:update` |
| Напоминания (Task > Create/Update/Delete) | `tasks:create`, `tasks:update`, `tasks:delete` |

Подробнее — в разделе [Авторизация](/api/authorization).

### 403 Forbidden при активации Pachca Trigger

**Причина:** узел попытался автоматически прописать Webhook URL, но у токена не хватает прав. Для **токена бота** нужен скоуп `bot_self:webhook:write` (саморегистрация методом [Саморегистрация вебхука бота](/api/bots/update-webhook)). Для **персонального токена** — скоуп `bots:write` и доступ редактора к указанному боту (регистрация методом [Редактирование бота](/api/bots/update)).
...
1. **Токен бота:** добавьте токену право `bot_self:webhook:write` — в карточке бота на вкладке **API** выберите **Редактировать** в меню токена
2. **Персональный токен:** убедитесь, что владелец токена может редактировать бота — он его создатель или подходит под настройку **Кто может редактировать настройки бота** — и что у токена есть скоуп `bots:write` (**Редактировать** в меню токена на странице **Интеграции** → **API**)
...
3. **URL не прописан в настройках бота** — в ручном режиме узел не трогает webhook-слот бота. Проверьте, что Production URL указан адресом исходящего вебхука в карточке бота (вкладка **Исходящий вебхук**). В автоматическом режиме убедитесь, что в узле указан корректный **Bot ID**
...
**Решение:** скопируйте Signing secret из карточки бота в Пачке (вкладка **Исходящий вебхук**) и вставьте в Credentials.
...
**Причина:** `trigger_id` из события `button_pressed` действителен только **3 секунды**. Если между получением вебхука и вызовом [Открытие представления](/api/views/open) проходит больше времени — форма не откроется.
