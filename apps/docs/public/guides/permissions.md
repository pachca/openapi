...
- **Только Владелец** — `chat_exports:read`, `chat_exports:write` для [экспорта сообщений](/guides/export), `audit_events:read` для [журнала аудита](/guides/audit-events), `company_chats:read` для [списка чатов пространства](/api/chats/list-company) и `company_bots:read` для [списка ботов пространства](/api/bots/list-company). Все они доступны на тарифе **Корпорация**.
