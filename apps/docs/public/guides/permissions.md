...
Исключения из правила есть, и все они описаны ниже: гости не работают с API вовсе, а Владелец пространства может получить перечень всех чатов и всех ботов, состав участников любого чата и выгрузить переписку всего пространства.
...
**Владелец не выделен отдельным значением роли.** В поле `role` у него `admin`, как и у остальных администраторов, и определить Владельца по объекту сотрудника нельзя. Редактировать его чужим токеном нельзя вообще: [Редактирование сотрудника](/api/users/update), [Загрузка аватара сотрудника](/api/users/update-avatar) и [Удаление сотрудника](/api/users/delete) возвращают для Владельца `403`. Своим токеном он меняет собственный профиль, но не может сменить себе роль и не может себя деактивировать.

**Гости не работают с API.** Создать токен, войти в CLI или авторизовать бота гость не может: права на работу с API у этой роли нет. Если токен был получен до смены роли, любой запрос вернёт `403`. Ограничение касается только его собственных токенов: сам гость остаётся полноценным участником чата, куда его пригласили, и другие токены видят его как обычного участника.

**Мульти-гость работает с API наравне с сотрудником**, но видит через него ровно то же, что в интерфейсе. Он создаёт персональные токены, входит в CLI и авторизует ботов, а создавать и настраивать ботов не может. Подробнее — в разделе [Мульти-гость в API](#multi-gost-v-api).

### Что доступно только Владельцу и Администратору

Большинство скоупов доступно всем ролям, кроме гостя. Исключения стоит знать заранее, потому что при создании токена такие скоупы просто не появятся в списке:

- **Только Владелец** — `chat_exports:read`, `chat_exports:write` для [экспорта сообщений](/guides/export), `audit_events:read` для [журнала аудита](/guides/audit-events), `company_chats:read` для метода [Список чатов пространства](/api/chats/list-company) и `company_bots:read` для метода [Список ботов пространства](/api/bots/list-company). Все они доступны на тарифе **Корпорация**.
- **Владелец и Администратор** — `users:create`, `users:update`, `users:delete`, `user_status:write`, `user_avatar:write`, `group_tags:write` для [тегов](/guides/tags).
...
- **Только персональный токен** — `bots:read` и `bots:write` для [управления ботами](/guides/bots/overview) и `drafts:read`, `drafts:write` для черновиков и отложенных сообщений — [Новый черновик](/api/drafts/create) и соседние методы. Скоупы ботов есть у Владельца, Администраторов и Сотрудников, скоупы черновиков — ещё и у мульти-гостя, а токену бота их не выдать вовсе: с ним метод отвечает `403`. То есть бот не может ни завести другого бота, ни написать черновик.

Полная таблица скоупов по ролям — в разделе [Авторизация](/api/authorization#dostupnye-skoupy).

### Мульти-гость в API

Мульти-гость видит в интерфейсе только чаты, куда его добавили, и их участников. API отдаёт ему ровно столько же:

- **Сотрудники** — [Список сотрудников](/api/users/list), [Информация о сотруднике](/api/users/get) и [Статус сотрудника](/api/users/get-status) отдают его самого, участников общих с ним чатов вместе с ботами и публичных ботов. Треды и архивные чаты общими не считаются, на остальных сотрудников методы отвечают `404`. У чужих сотрудников `list_tags` приходит пустым, а почта и телефон — `null`.
- **Чаты** — [Список чатов](/api/chats/list) с `availability=public` отдаёт только открытые чаты, где он участник. Создать беседу или канал он не может: метод [Новый чат](/api/chats/create) отвечает `403`, как и в интерфейсе.
- **Напоминания** — [Список напоминаний](/api/tasks/list) с `chat_ids` отдаёт напоминания только тех чатов, где он участник, и доступных ему тредов: открытые чаты, куда он не вступал, не учитываются.
- **Теги** — [Список тегов](/api/group-tags/list) и [Информация о теге](/api/group-tags/get) отдают только теги активных бесед и каналов, где он участник. [Список сотрудников тега](/api/group-tags/list-users) ему закрыт и отвечает `403`.
- **Упоминания** — `<@id>` в методах [Новое сообщение](/api/messages/create) и [Редактирование сообщения](/api/messages/update) превращается в упоминание только для доступных ему сотрудников, остальные остаются в тексте как есть.
- **Черновики** — черновики и отложенные сообщения доступны ему, как сотруднику: скоупы `drafts:read` и `drafts:write` выдаются и мульти-гостю.
- **Боты** — скоупы `bots:read` и `bots:write` ему не выдаются, поэтому создавать и настраивать ботов он не может. Авторизовать бота и подключить его из [витрины](/guides/bots/showcase) — может. Добавлять ботов в чаты он не может ни при какой настройке бота.
...
> **Внимание:** Роль в чате **не возвращается** в ответе метода [Список участников чата](/api/members/list). Поле `role` в объекте сотрудника — это его роль в пространстве, а не в чате. Чтобы получить участников с конкретной ролью в чате, используйте одноимённый параметр запроса `role` со значением `owner`, `admin`, `editor` или `member`.
...
Участника, который попал в чат по [тегу](/guides/tags), исключение из чата не выводит: его участие задаёт тег. Чтобы такой сотрудник вышел, снимают тег с него или отвязывают тег от чата.
...
Поэтому по роли в пространстве нельзя предсказать права в конкретном чате. Проверить, кто в чате Создатель, Админ, Редактор или рядовой участник, можно параметром `role` в методе [Список участников чата](/api/members/list).
...
Гости и мульти-гости видят только те чаты, в которые их добавили. Открытые чаты пространства им недоступны. Как это правило действует в API для мульти-гостя — в разделе [Мульти-гость в API](#multi-gost-v-api).
...
- **Перечень чатов всего пространства.** [Список чатов пространства](/api/chats/list-company) отдаёт беседы и каналы, включая закрытые, где Владелец не состоит. Личные переписки и треды в выдачу не попадают. Нужен тариф **Корпорация** и скоуп `company_chats:read`.
...
Граница у такой выгрузки одна: **закрытые беседы и каналы, где токен не участник, в списке не возвращаются**. Для Владельца пространства эту границу снимает отдельный метод — [Список чатов пространства](/api/chats/list-company):
...
> У инвентаризации отдельный [лимит](/api/limits) — около 30 запросов в минуту на токен, строже общего. Обход большого пространства нужно растягивать во времени.
...
Добавить их в чат методом [Добавление пользователей](/api/members/add) может:
...
Найти чаты, созданные конкретным сотрудником, можно по полю `owner_id` в выдаче метода [Список чатов](/api/chats/list), а его текущий статус проверить полем `suspended` в методе [Информация о сотруднике](/api/users/get).

## Видимость ботов

Видимость бота определяется правом на его редактирование, а не участием в чатах. [Список ботов](/api/bots/list) возвращает только тех ботов, которых вам разрешено редактировать: созданных вами и тех, чьи настройки открывают вам доступ. Это правило действует для всех ролей, включая Администратора пространства.

### Инвентаризация ботов пространства

Полный перечень ботов пространства отдаёт [Список ботов пространства](/api/bots/list-company):
...
| Добавить участника | [Добавление пользователей](/api/members/add) | Участие в закрытом чате. В открытый чат добавить может любой сотрудник пространства. Гости и мульти-гости добавлять не могут. |
...
| Исключить участника | [Исключение пользователя](/api/members/remove) | Роль Создателя или Админа в этом чате. Исключить Создателя нельзя. |
...
| Экспортировать сообщения | [Экспорт сообщений](/api/chats/request-export) | Роль Владельца пространства, тариф **Корпорация** |
| Прочитать журнал аудита | [Журнал аудита событий](/api/security/list) | Роль Владельца пространства, тариф **Корпорация** |
| Получить все чаты пространства | [Список чатов пространства](/api/chats/list-company) | Роль Владельца пространства, тариф **Корпорация** |
| Получить всех ботов пространства | [Список ботов пространства](/api/bots/list-company) | Роль Владельца пространства, тариф **Корпорация** |
