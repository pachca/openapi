...
Укажите название чата и список участников. Идентификаторы сотрудников можно получить через метод [Список сотрудников](/api/users/list), а идентификаторы тегов — через метод [Список тегов](/api/group-tags/list). Запросы с телом отправляются в формате JSON с заголовком `Content-Type: application/json`.
