...
Комментарий — это обычное сообщение через [Новое сообщение](POST /messages) с `entity_type: "thread"` и `entity_id` равным `thread.id`:

```bash title="Комментарий в тред"
...
Альтернативный способ — передать только `entity_id` равным `thread.chat_id`, без `entity_type`. Чат треда — обычный чат, и сообщение, отправленное в него, окажется в треде.
...
Метод [Список тредов](GET /threads) возвращает треды, в которых вы участвуете напрямую (участник чата треда) **или** в которых вы участвуете через родительский чат (участник того чата, где тред был создан).

> Публичные дискуссии, в которых вы **не состоите**, не попадают в выдачу — даже если в них есть открытые треды. Чтобы бот получал такой тред в этом списке, его нужно явно добавить либо в сам тред, либо в родительский чат.
...
В интерфейсе список участников треда отображается отдельно от участников родительского чата.
...
- [Доступы бота к чатам и сообщениям](/guides/bots/access#tredy) — детальные правила доступа бота к тредам и личным сообщениям
- [AI-агенты в Пачке](/guides/ai-agents/overview) — почему тред — естественная единица работы для агента и как использовать сквозные треды для координации
- [Исходящие вебхуки](/guides/webhook/overview) — события, которые бот получает из треда
- [Журнал аудита](/guides/audit-events) — событие `thread_created` фиксируется в журнале аудита
