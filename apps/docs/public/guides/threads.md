...
- Подключать к обсуждению сотрудников из других команд по контексту, не раскрывая историю канала
- Подключать **бота** к конкретному треду, не добавляя его в родительский чат — точечная автоматизация без расширения прав
- Точно делегировать вопрос конкретному сотруднику, не пересылая сообщение
...
Самостоятельный тред не привязан к сообщению — его создают методом [Новый самостоятельный тред](/api/threads/create). Тело запроса не требуется, создатель треда становится его единственным участником, а поля `message_id` и `message_chat_id` в ответе возвращаются как `null`. Метод требует скоуп `threads:create`.

**Создание самостоятельного треда**
...
Комментарий в тред — это обычное сообщение, отправленное методом [Новое сообщение](/api/messages/create). Есть два равнозначных способа адресации.
...
Чтобы получить все комментарии треда — вызовите [Список сообщений чата](/api/messages/list) с `chat_id` равным `thread.chat_id`.
...
|------|----------------|-------------|
| `id` | Идентификатор треда как сущности | В методе [Информация о треде](/api/threads/get) |
| `chat_id` | Идентификатор **чата треда** — отдельного чата, в котором хранятся комментарии треда | Для отправки комментария [Новое сообщение](/api/messages/create) и для чтения [Список сообщений чата](/api/messages/list) |
| `message_id` | Идентификатор родительского сообщения, к которому привязан тред. `null` у самостоятельного треда. | При создании треда у сообщения — в методе [Новый тред](/api/threads/add) |
...
Метод [Список тредов](/api/threads/list) возвращает все доступные вам треды:
...
У треда, как у самостоятельного чата, есть свой список участников, отдельный от родительского чата. Получить его можно методом [Список участников чата](/api/members/list) с `chat_id` треда.

Добавить участника в тред можно тремя способами:

- **Через API** — метод [Добавление пользователей](/api/members/add), передав `chat_id` **треда** (не родительского чата). Если добавляемый сотрудник не состоит в родительском чате, он станет участником сквозного треда: будет видеть тред и родительское сообщение, но не остальную историю.
...
- **Через упоминание тега** в комментарии треда — в тред попадут все сотрудники [тега](/guides/tags), даже те, кто не состоит в родительском чате.

> Если нужно упомянуть участника в треде, но **не** добавлять его автоматически — передайте `skip_invite_mentions: true` в теле метода [Новое сообщение](/api/messages/create). Упоминание останется в тексте, но в участники треда сотрудник не попадёт. Полезно, когда бот ссылается на сотрудника для контекста, не втягивая его в обсуждение.


Удаление участника и выход из треда работают так же, как для обычных чатов: методы [Исключение пользователя](/api/members/remove) и [Выход из чата](/api/members/leave) с `chat_id` треда.
