...
Включите настройку **Сохранять историю событий** на вкладке **Исходящий Webhook** в настройках бота (подробнее — в разделе [Настройка и типы событий](/guides/webhook/events#obschie-nastroyki)), чтобы получать события через метод [История событий](/api/bots/list-events). Без неё метод отдаёт пустой список. Включить настройку можно и программно — полем `events_history_enabled` в методах [Новый бот](/api/bots/create) и [Редактирование бота](/api/bots/update).
