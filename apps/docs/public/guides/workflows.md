...
1. Получи список чатов, найди нужный по полю `name`
...
1. Для каждого файла: получи параметры загрузки
2. Для каждого файла: подставь имя файла вместо `${filename}` в `key`, затем загрузи файл POST на `direct_url`
3. Собери массив `files` из всех загруженных файлов (`key`, `name`, `file_type`, `size`)
4. Отправь сообщение со всеми файлами
...
> Альтернатива GET /users?query= с более точным ранжированием.
