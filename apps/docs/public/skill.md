...
- [CLI: Прямые запросы](https://dev.pachca.com/guides/cli/api-requests) — Вспомогательная команда pachca api для прямых HTTP-запросов, когда типизированной команды не хватает: поля -f/-F, инлайн JSON, stdin. Поиск эндпоинта: ls, --describe, --spec, --docs.
