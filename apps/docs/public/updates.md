...
- `GET /profile/status` и `GET /users/{user_id}/status`: тело ответа описано верно — при отсутствующем статусе приходит `null`, а не обёртка с `data: null`
- `display_avatar_url` и `display_name` у сообщения больше не обязательные: их нет в ответе для сообщений от сотрудников
- `Reaction.name` больше не nullable — `null` в этом поле не приходит
