...
Добавлены официальные SDK для 5 языков — [TypeScript](/api/sdk#typescript), [Python](/api/sdk#python), [Go](/api/sdk#go), [Kotlin](/api/sdk#kotlin) и [Swift](/api/sdk#swift). Все SDK автоматически генерируются из OpenAPI-спецификации и включают типизированные клиенты, автопагинацию и автоматические повторные запросы при `429`.
...
Документация реструктурирована: [Быстрый старт](/api/quickstart), [Пагинация](/api/pagination), [Загрузка файлов](/api/file-uploads), [Запросы и ответы](/api/requests-responses) вынесены в отдельные страницы, раздел форм разбит на три страницы.

В вебхук [Разворачивание ссылок](/guides/link-previews) добавлено поле `user_id` — идентификатор отправителя сообщения со ссылкой.
