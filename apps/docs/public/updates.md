...
- Ответ `409` в методах `Новый сотрудник` (`POST /users`), `Редактирование сотрудника` (`PUT /users/{id}`), `Новый тег` (`POST /group_tags`), `Редактирование тега` (`PUT /group_tags/{id}`) и `Закрепить сообщение` (`POST /messages/{id}/pin`)
