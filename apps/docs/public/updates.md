...
- `first_name` у сотрудника и `value` у дополнительного поля стали опциональными: API возвращает `null`, пока приглашение не принято и пока поле не заполнено. Раньше на этом падал разбор ответа `GET /users` в Swift, Kotlin, Go и C#
- Примеры из README снова компилируются: разбор события видеозвонка в истории вебхуков, актуальное имя поля `Content-Disposition` при загрузке файла и вызов stub-клиента в TypeScript
...
- `User` → `Update Avatar`: адрес запроса собирался из объекта выбора сотрудника, а не из его идентификатора, и операция не работала
- `Chat` → `Download Export`: та же причина, выгрузка не скачивалась при выборе чата из списка или по ссылке
- Выбор сотрудника: список открывается сразу, а не остаётся пустым до ввода текста, как это уже работает у чатов
