...
В payload трёх исходящих вебхуков (`reaction_new`, `reaction_delete`, `view_submit`) добавлено поле `chat_id` — идентификатор чата, в котором произошло событие. Поле всегда присутствует в payload, в редких случаях может прийти как `null`. Это убирает необходимость в дополнительном запросе [`GET /messages/{id}`](GET /messages/{id}) для определения чата по событию.

В вебхуке о заполнении формы значение `chat_id` фиксируется в момент **открытия** формы (нажатия кнопки), а не отправки — если форма провисела открытой длительное время, поле всё равно ссылается на чат с кнопкой. Для форм, открытых до выкатки этого изменения, `chat_id` придёт как `null` — такие формы вымоются по TTL сохранённого представления (30 дней).
...
- Сборка: добавлен `prebuild`, удаляющий `e2e/.n8n-test/` — устраняет падение `ENAMETOOLONG` после локальных E2E-прогонов
