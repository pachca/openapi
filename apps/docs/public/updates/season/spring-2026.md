...
Поле `link_preview` в запросе метода [Новое сообщение](/api/messages/create) перенесено на верхний уровень тела запроса — отдельно от объекта `message`.
...
Поле `thread` в модели [сообщения](/api/models#soobschenie) теперь содержит только `id` и `chat_id`. Полная модель треда (с `message_id`, `message_chat_id`, `updated_at`) возвращается в методах [Новый тред](/api/threads/add) и [Информация о треде](/api/threads/get).
