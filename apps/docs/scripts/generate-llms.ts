...
function generateLlmsTxt(api: Awaited<ReturnType<typeof parseOpenAPI>>) {
  ...
  content += '## SDK\n\n';
  content += 'Типизированные клиенты для 5 языков. Все следуют единому паттерну: `PachcaClient(token)` → `client.service.method(request)`.\n\n';
  ...
  content += 'Swift:      PachcaClient(token: "TOKEN") → try await pachca.messages.createMessage(...)\n';
  content += '```\n\n';
...
async function generateLlmsFullTxt(api: Awaited<ReturnType<typeof parseOpenAPI>>) {
  ...
  content += '# SDK\n\n';
  content += 'Типизированные клиенты для 5 языков. Единый паттерн: `PachcaClient(token)` → `client.service.method(request)`.\n\n';
  content += '| Язык | Пакет | Установка |\n';
  content += '|------|-------|----------|\n';
  content += '| TypeScript | `@pachca/sdk` | `npm install @pachca/sdk` |\n';
  content += '| Python | `pachca` | `pip install pachca` |\n';
