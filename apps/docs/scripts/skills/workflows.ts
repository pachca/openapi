  ...
  'pachca-forms': [
    {
      ...
      steps: [
        'Отправь сообщение с кнопкой (POST /messages с buttons)',
        'При нажатии кнопки — получи вебхук-событие с trigger_id',
        'Открой форму: POST /views/open с trigger_id, type: "modal" и описанием формы (view.title, view.blocks)',
        'Пользователь заполняет форму → результат приходит на Webhook URL',
      ],
      notes:
        'Формы работают только от бота. Проверь: GET /profile → data.bot === true. trigger_id приходит в вебхук-событии нажатия кнопки. Время жизни — 3 секунды. Без него POST /views/open не сработает.',
    },
  ],
  'pachca-users': [
    {
