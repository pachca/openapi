...
curl "https://api.pachca.com/api/shared/v1/messages" \\
  ...
  -d '{"chat":{"name":"Проект Alpha","channel":false,"group_tag_ids":[42],"member_ids":[186,187]}}'`,
    ...
    {
      title: 'Синхронизировать участников чата с тегом',
      steps: [
        'GET /group_tags/{id}/users (с пагинацией) — получи всех пользователей тега',
        'GET /chats/{id}/members (с пагинацией) — получи текущих участников чата',
        'Вычисли разницу: кого добавить (в теге, но не в чате), кого удалить (в чате, но не в теге)',
        'POST /chats/{id}/members с member_ids для добавления',
        'DELETE /chats/{chatId}/members/{userId} для каждого удаляемого',
      ],
      notes: 'Учитывай пагинацию — оба списка могут быть больше 50 элементов.',
    },
    {
  ...
  'pachca-forms': [
    ...
    },
    {
      ...
      steps: [
        'Отправь сообщение с кнопкой «Пройти опрос» в канал или ЛС: POST /messages с buttons[0][0].data = "survey_start"',
  ...
  'pachca-users': [
    ...
    },
    {
      ...
      steps: [
        ...
        'POST /messages с entity_type: "user", entity_id: user.id — отправить welcome-сообщение в ЛС',
      ],
  ...
  -d '{"task":{"kind":"reminder","content":"Позвонить клиенту","due_at":"2026-03-01T10:00:00Z"}}'`,
    },
    {
      ...
      steps: [
        'GET /tasks с фильтром status: undone и пагинацией',
        'Задачи возвращаются в порядке due_at (ближайшие сначала)',
      ],
      curl: `curl "https://api.pachca.com/api/shared/v1/tasks?status=undone&limit=50" \\
  -H "Authorization: Bearer $TOKEN"`,
    },
    {
  ...
  -H "Authorization: Bearer $TOKEN"`,
    },
    {
      ...
      steps: [
        'GET /custom_properties — список всех дополнительных полей пространства (id, name, тип)',
