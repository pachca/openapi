...
function findParamInSchema(schema: Schema | undefined, paramPath: string): boolean {
  ...
  for (const variant of [...(schema.allOf || []), ...(schema.anyOf || []), ...(schema.oneOf || [])]) {
        ...
        {currentResponse.description && (
          <div className='pt-6 mb-2'>
            <div className="flex items-start gap-4 p-4 rounded-xl bg-background-tertiary border border-background-border">
              <div className={`shrink-0 w-2 h-2 mt-2 rounded-full ${isError ? 'bg-method-delete' : 'bg-method-post'}`} />
              <div className="space-y-1">
                <div className="flex items-baseline gap-2">
                  <span className={`text-[14px] font-bold font-mono ${isError ? 'text-method-delete' : 'text-method-post'}`}>
                    {activeCode}
                  </span>
