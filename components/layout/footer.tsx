      ...
      <div className={`${noMargin ? 'mt-2' : 'mx-8 xl:mx-10'} pt-10`}>
        {adjacent && (adjacent.prev || adjacent.next) && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
            {adjacent.prev ? (
              <PrevButton item={adjacent.prev} />
            ) : (
              <div />
            )}

            {adjacent.next && (
              <NextButton item={adjacent.next} />
            )}
          </div>
        )}

        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-[13px] text-text-tertiary">
          <div>© 2026 <a href="https://pachca.com" className="font-medium hover:text-primary transition-colors" target="_blank" rel="noopener noreferrer">Пачка</a>. Сделано с заботой.</div>
          <div className="flex items-center gap-6">
            <a href="https://pachca.com/security/bug-bounty" className="font-medium hover:text-primary transition-colors" target="_blank" rel="noopener noreferrer">Bug bounty</a>
            <a href="https://pachca.com/help-center" className="font-medium hover:text-primary transition-colors" target="_blank" rel="noopener noreferrer">База знаний</a>
            <a href="https://pachca.com/glossary" className="font-medium hover:text-primary transition-colors" target="_blank" rel="noopener noreferrer">Глоссарий</a>
          </div>
        </div>
      </div>
    </footer>
