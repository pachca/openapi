...
- [ ] Версия объявлена в `releases.json` (и в собственном changelog у CLI/n8n) по уровню изменения:
  ...
  - Ломающее изменение **любого** масштаба выносится в запись отдельным пунктом
    типа `~` первой строкой, с прямой формулировкой, что именно перестанет
    работать. Иначе поломка теряется в списке добавленных полей.
