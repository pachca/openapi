...
| `apps/docs/scripts/skills/workflows.ts` | Пошаговые сценарии (title, steps, notes, curl) | Новый/изменённый workflow, новый параметр влияющий на шаги |
...
- [ ] Если нужен новый workflow — добавить в `workflows.ts` → `WORKFLOWS[skillName]`
...
- Обновления с датой < 14 дней назад получают badge «Новое» на сайте
...
> **Не подтверждено.** Значения ниже перенесены из старых приватных
> заметок (снимок ~2026-05-04) и **не верифицированы** против бэкенда при
> создании этого файла. Первый же аудит по этому процессу должен
> подтвердить хеш (`git cat-file -e <sha>` в бэкенд-репо) или взять
> актуальный `origin/master` как новую базу и перезаписать секцию.

- **Last checked:** 2026-05-04 _(из приватных заметок, не верифицировано)_
- **Last commit:** `9bd5a3c2b` _(из приватных заметок, не верифицировано)_
