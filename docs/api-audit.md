...
| **SDK**       | `packages/spec/typespec.tsp` (`version: "X.Y"`) + `releases.json` (product `sdk`, одна запись на все языки) | Поднять `version` в typespec при необходимости; запись в releases.json. **Per-language `package.json` не трогать** (placeholder `0.0.0`). Публикуются все 6 языков вместе             |
