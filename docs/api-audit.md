...
**Сверять метод целиком, а не дифф коммита.** Дифф показывает, что изменилось, но не показывает, совпадает ли модель с методом: расхождение, появившееся раньше, в нём не видно вовсе. То же правило, что для роутов в шаге 1, и по той же причине. Реальный случай (аудит 2026-08-05): в `bots_controller` разбирался дифф BAK-2981, из него был вынесен один факт — «webhook стал необязательным». В том же методе рядом лежали ещё два, оба пропущены: новый источник `name`/`nickname` на корне объекта и жёсткое требование обёртки. А самой обёртки не было в спеке **изначально**, хотя у соседних ресурсов она есть, — диффом это не ловится в принципе.
...
- [ ] Версия объявлена в `releases.json` (и в собственном changelog у CLI/n8n) по уровню изменения:
  ...
  Пример решения: SDK 1.1.0 (2026-08-01) — `ip_address`/`user_agent` в записи
  журнала аудита стали nullable, а модель деталей видеозвонка разделена на две.
  Для потребителя на Go, Swift, Kotlin и TypeScript это ломающее изменение, но
  журнал аудита доступен только владельцу пространства на тарифе «Корпорация»,
  поэтому выпущено минором, а не мажором. `package.json` пакета при этом не
  трогаем — версию туда проставляет CI из `releases.json`.
