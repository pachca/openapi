...
| **SDK**       | `releases.json` (product `sdk`, одна запись на все языки)                | Указать следующую semver-версию в releases.json (как у всех; от `latest` dist-tag `@pachca/sdk` = `1.0.19` → `1.0.20`). **Per-language `package.json` не трогать** (placeholder `0.0.0`). Публикуются все 6 языков вместе. `typespec.tsp` — источник спеки, не версии SDK |
