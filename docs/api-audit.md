...
приватный репозиторий, клонируется локально — путь зависит от машины) и
...
>
> Документ намеренно описывает процесс на уровне ролей и назначения, без
> конкретных путей, классов и имён внутренней структуры бэкенда: тот, кто
> запускает аудит, всегда имеет приватный код бэкенда локально и находит
> точные файлы исследованием самого бэкенда. Этого документа достаточно,
> чтобы выполнить аудит, не обращаясь к чьим-либо личным заметкам.

**Точка отсчёта аудита** — последняя дата в `apps/docs/content/updates.mdx`
(публичная история изменений API). Отдельный чекпоинт по коммиту бэкенда
не ведётся: аудит просматривает изменения публичного API бэкенда начиная
с этой даты включительно. Уже отражённое в spec / уже выпущенное агент
видит и повторно не вносит. См. секцию «Точка отсчёта аудита» в конце.
...
> Был случай: создана новая CLI-версия при том, что предыдущая ещё не была опубликована в npm → запись свернули в pending-версию, а отдельный датированный блок в `updates.mdx` слили в один с датой релиза. Для пакетов, которые в тот же аудит уже были выпущены, новые версии корректны. Урок: всегда проверять реальную публикацию перед bump'ом, независимо по каждому пакету.
...
> workflow обязателен. Был случай, когда изменение вводило ограниченный
> многошаговый сценарий (специфичная роль, обязательное предусловие по
> связке параметров, специфичная ошибка `400`) и его забыли вынести в
> workflow — добавили постфактум по замечанию. Теперь это обязательный шаг.
...
| Внутренние коды ошибок непубличного API | Не документируются; конкретный список не приводится |
| Внутренние, бета- и служебные эндпоинты | Не входят в публичный API; в область аудита не входят и не документируются (механизм/клиент/флаги намеренно не описываются) |
...
| Внутренние, бета- и служебные эндпоинты | Не входят в публичный API; в область аудита не входят и не документируются — конкретный механизм, клиент и флаги намеренно не приводятся |
...
Конкретные пути/имена в бэкенде не приводятся намеренно — тот, кто
запускает аудит, имеет код бэкенда локально и находит их по роли.
...
прогоном самого аудита по шагам выше против текущей основной ветки
...
## Точка отсчёта аудита

Отдельный чекпоинт по коммиту бэкенда **не ведётся**. Точка отсчёта — **последняя дата** из блоков `<!-- update:YYYY-MM-DD -->` в начале `apps/docs/content/updates.mdx` (публичная история изменений API).

- **Перед аудитом** — взять эту дату и просмотреть изменения публичного API основной ветки бэкенда начиная с неё включительно (шаг 0).
- Захват уже отражённого в spec / уже выпущенного — нормально: агент это видит и повторно не вносит, дублирования не будет, поэтому дату можно брать с запасом.
- **По завершении** — новые user-facing изменения попадают в `updates.mdx` с текущей датой; она же становится точкой отсчёта следующего аудита. Хранить коммит/дату сверки отдельно не нужно.
