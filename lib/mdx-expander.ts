...
export async function expandMdxComponents(content: string): Promise<string> {
  ...
  if (result.includes('<ErrorSchema')) {
    ...
    if (apiError) {
      errorSchemaMarkdown += formatSchemaWithTitle(apiError, 'ApiError (400, 403, 404, 409, 410, 422)');
      errorSchemaMarkdown += '\n';
    }
  ...
  result = result.replace(/<CodeBlock\s+language="([^"]*)"(?:\s+title="([^"]*)")?>([\s\S]*?)<\/CodeBlock>/g, (_, lang, title, code) => {
