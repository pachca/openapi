...
export const HTTP_CODES = [
  ...
  { code: '301', message: 'Moved Permanently', description: 'Запрошенный ресурс был на постоянной основе перемещён в новое месторасположение (такой ответ вы можете получить если выполните запрос по протоколу HTTP, а не по HTTPS)' },
  { code: '400', message: 'Bad Request', description: 'Неприемлемый запрос (часто из-за отсутствия обязательного параметра)' },
  { code: '401', message: 'Unauthorized', description: 'Предоставлен недействительный токен доступа' },
  { code: '402', message: 'Payment Required', description: 'Параметры действительны, но запрос не выполнен' },
  { code: '403', message: 'Forbidden', description: 'Предоставленный токен доступа не имеет разрешений на выполнение запроса' },
  { code: '404', message: 'Not Found', description: 'Запрашиваемый ресурс не существует' },
  { code: '409', message: 'Conflict', description: 'Запрос конфликтует с другим запросом (возможно, из-за использования того же идемпотентного ключа)' },
  { code: '422', message: 'Unprocessable Entity', description: 'С запросом все хорошо, но правила сервиса не позволяют его обработать (например, при попытке создания контакта с уже существующим номером телефона в базе)' },
  { code: '429', message: 'Too Many Requests', description: 'Слишком много запросов слишком быстро попадают в API' },
  { code: '500, 502, 503, 504', message: 'Server Errors', description: 'Что-то пошло не так на сервере Пачки (это редкость)' },
];
