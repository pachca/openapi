...
- **Добавлено** (threads list): Новая команда — список тредов, доступных текущему пользователю или боту. Поддерживает фильтрацию по времени последнего сообщения (`--last-message-at-after` / `--last-message-at-before`), курсорную пагинацию и сортировку по убыванию времени последнего сообщения. Публичные дискуссии, в которых пользователь не состоит, не возвращаются — чтобы тред попал в список, бот должен быть в тред-чате или в родительской дискуссии
