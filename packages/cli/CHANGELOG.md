...
- **Добавлено** (upgrade): Команда `pachca upgrade` — обновление CLI до последней версии с детекцией пакетного менеджера (npm, bun, pnpm, yarn, homebrew)
- **Исправлено** (init hook): Ошибки авторизации теперь выводятся в JSON при `-o json` (раньше формат игнорировался)
- **Исправлено** (update banner): Баннер обновления показывается даже при ошибках команд
