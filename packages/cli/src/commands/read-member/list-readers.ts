...
export default class ReadMemberListReaders extends BaseCommand {
  ...
  static apiPath = "/messages/{id}/read_member_ids";
  ...
  async run(): Promise<void> {
    ...
    if (flags.all) {
      ...
      while (pages < 500) {
        ...
        const response = await this.apiRequest({ method: 'GET', path: `/messages/${args.id}/read_member_ids`, query });
    ...
    const { data } = await this.apiRequest({
      method: 'GET',
      path: `/messages/${args.id}/read_member_ids`,
      query: {
      limit: flags.limit,
      cursor: flags.cursor,
      },
