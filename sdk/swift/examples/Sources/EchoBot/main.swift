...
fflush(stdout)
let created = try await pachca.messages.createMessage(.init(
...
let fetched = try await pachca.messages.getMessage(.init(path: .init(id: messageId)))
print("  ✓ Fetched: \"\(fetched.data.content)\"")

// ── Step 3: Add a reaction ─────────────────────────────────────

print("Step 3: Adding reaction...")
let reaction = try await pachca.reactions.addReaction(.init(
...
let thread = try await pachca.threads.createThread(.init(path: .init(id: messageId)))
print("  ✓ Created thread #\(thread.data.id) (chat_id: \(thread.data.chat_id))")

// ── Step 5: Reply in thread ────────────────────────────────────

print("Step 5: Replying in thread...")
let reply = try await pachca.messages.createMessage(.init(
    body: .json(.init(message: .init(
...
try await pachca.messages.pinMessage(.init(path: .init(id: messageId)))
...
let edited = try await pachca.messages.updateMessage(.init(
...
try await pachca.messages.unpinMessage(.init(path: .init(id: messageId)))
