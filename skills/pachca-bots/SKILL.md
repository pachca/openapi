...
name: pachca-bots
description: >
  Управляет ботами, входящими/исходящими вебхуками и разворачиванием ссылок
  (unfurling). Применяется для: настройки бота, обработки вебхуков, проверки
  подписи вебхука, разворачивания ссылок. Не применяется для: отправки сообщений
  от бота (→ pachca-messages), интерактивных форм (→ pachca-forms).
---
...
```bash
# Пример: ежедневный отчёт в канал #analytics (chat_id: 12345)
curl "https://api.pachca.com/api/shared/v1/messages" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"message":{"entity_id":12345,"content":"📊 Отчёт за день:\n- Продажи: 42\n- Новых пользователей: 7"}}'
```
...
4. При нажатии кнопки — обработай callback (шаг выше) и обнови статус алерта

```bash
curl "https://api.pachca.com/api/shared/v1/messages" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"message":{"entity_id":99901,"content":"🔴 Production down: сервис payments недоступен","buttons":[[{"text":"Взять в работу","data":"alert_ack_501"},{"text":"Игнорировать","data":"alert_ignore_501"}]]}}'
```
