...
2. При появлении ссылки бот получает вебхук `"event": "link_shared"` с массивом `links`. Каждый объект содержит `url`, `domain` и `skip` (boolean — если `true`, автор скрыл превью и бот должен пропустить эту ссылку)

3. Извлеки данные из своей системы по URL из `links` (пропуская ссылки с `skip: true`)
