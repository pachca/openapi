...
description: >
  Отправляет сообщения в каналы, беседы и личные чаты. Поддерживает ответы в
  треды, загрузку файлов, кнопки, реакции, закрепление, прочтения. Применяется
  для: отправки сообщений, ответов в треды, прикрепления файлов, реакций, истории
  чата, закрепления. Не применяется для: создания каналов (→ pachca-chats),
  управления ботами (→ pachca-bots).
---
