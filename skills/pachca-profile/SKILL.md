...
name: pachca-profile
description: >
  Предоставляет доступ к профилю текущего пользователя: обновление статуса,
  кастомные поля сотрудников. Применяется для: получения своего профиля,
  обновления статуса, работы с дополнительными полями. Не применяется для:
  управления другими сотрудниками (→ pachca-users).
---
