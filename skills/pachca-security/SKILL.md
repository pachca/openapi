...
```bash
curl "https://api.pachca.com/api/shared/v1/audit_events?start_time=$DATE_FROM&end_time=$DATE_TO&limit=50" \
...
1. GET /audit_events с фильтром `"event_key": "user_2fa_fail"` (или `"user_login"`) за нужный период
...
1. GET /audit_events с параметрами `start_time` и `end_time` (ISO 8601)
